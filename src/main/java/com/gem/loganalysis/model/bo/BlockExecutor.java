...
//        System.out.println("block result: " + block(dto));
    ...
    public static Boolean block(BlockExecuteHelper info) {
        ...
        if (CollUtil.isNotEmpty(dataSet)) {
            for (HashMap<String, String> map : dataSet) {
                BlockExecuteDTO executeDTO = new BlockExecuteDTO(map);
                block(executeDTO);
            }
        }
        return true;
     ...
     * @param dto 参数
     */
    private static void block(BlockExecuteDTO dto) {
        ...
        if (StrUtil.isNotEmpty(dto.getBlockCommand())) {
            ...
            System.out.println("Block result: " + sshAgent.exec(blockCommand));
        } else {
            log.warn("封堵命令模板为空!");
        }
    ...
    private static void saveBlockRecord(String assetId, String eventId, String blockIp, String blockType, String blockDuration) {
        ...
        insertSql.append("'").append(IdUtil.fastSimpleUUID()).append("',").append("'").append(blockIp).append("',").append("'").append(blockType).append("',").append("0,").append("'system',").append("'").append(DateUtil.format(new Date(), DatePattern.PURE_DATETIME_PATTERN)).append("',").append("'").append(DateUtil.format(endTime, DatePattern.PURE_DATETIME_PATTERN)).append("',").append("1,").append("'").append(assetId).append("',").append("'").append(eventId).append("')");
